...
// Default parameter values including image size can be changed in `config.xml.old`
...
function readPreferencesFromCfg(cfg) {
    try {
        ...
        var msg = '[Browser][SplashScreen] Error occured on loading preferences from config.xml.old: ' + JSON.stringify(e);
        console.error(msg);
    }
}
...
/**
 * Tries to read config.xml.old and override default properties and then shows and hides splashcreen if it is enabled.
 */
